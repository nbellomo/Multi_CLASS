...
int spectra_indices(
  ...
  if (ppt->has_cls == _TRUE_) {
    ...
    /** Change to ptr->selection_num*ppt->selection_tracers_number 
     *  once you compute also auto-tracer correlations. */
    if ((ppt->has_scalars == _TRUE_) &&
    ...
    if ((ppt->has_cl_number_count == _TRUE_) && (ppt->has_scalars == _TRUE_)) {
      ...
      if (psp->selection_multitracing == _TRUE_) {
		index_ct += (psp->d_size*(psp->d_size-1)+(psp->d_size-psp->non_diag)*(psp->d_size-1-psp->non_diag))/2;
	  }
    }
    else {
    ...
    */
    ...
    if (ppt->has_scalars == _TRUE_) {
      ...
      if (psp->has_dd == _TRUE_) {
        if (psp->selection_multitracing == _FALSE_) {
		  ...
		  for (index_ct=psp->index_ct_dd;
               index_ct<psp->index_ct_dd+psp->d_size*psp->d_size-(psp->d_size-psp->non_diag)*(psp->d_size-1-psp->non_diag);
               index_ct++)
            psp->l_max_ct[ppt->index_md_scalars][index_ct] = ppt->l_lss_max;
		}
	  }
      ...
      for (index_d1=0; index_d1<psp->d_size; index_d1++) {
        ...
        if (ppt->has_nc_density == _TRUE_) {
          ...
          if (psp->selection_multitracing == _FALSE_)
            ...
            transfer_ic2_nc[index_d1] += transfer_ic2[ptr->index_tt_density+ptr->selection_num+index_d1];
        }
        ...
        if (ppt->has_nc_rsd     == _TRUE_) {
          ...
          if (psp->selection_multitracing == _FALSE_)
            ...
            transfer_ic2_nc[index_d1]
              += transfer_ic2[ptr->index_tt_rsd+ptr->selection_num+index_d1]
              + transfer_ic2[ptr->index_tt_d0+ptr->selection_num+index_d1]
              + transfer_ic2[ptr->index_tt_d1+ptr->selection_num+index_d1];
        }
        ...
        if (ppt->has_nc_lens == _TRUE_) {
          ...
          if (psp->selection_multitracing == _FALSE_)
            ...
            transfer_ic2_nc[index_d1] +=
              psp->l[index_l]*(psp->l[index_l]+1.)*transfer_ic2[ptr->index_tt_nc_lens+ptr->selection_num+index_d1];
        }
        ...
        if (ppt->has_nc_gr == _TRUE_) {
          ...
          if (psp->selection_multitracing == _FALSE_)
            ...
            transfer_ic2_nc[index_d1]
              += transfer_ic2[ptr->index_tt_nc_g1+ptr->selection_num+index_d1]
              + transfer_ic2[ptr->index_tt_nc_g2+ptr->selection_num+index_d1]
              + transfer_ic2[ptr->index_tt_nc_g3+ptr->selection_num+index_d1]
              + transfer_ic2[ptr->index_tt_nc_g4+ptr->selection_num+index_d1]
              + transfer_ic2[ptr->index_tt_nc_g5+ptr->selection_num+index_d1];
        }
    ...
    */
    ...
    if (_scalars_ && (psp->has_dd == _TRUE_)) {
      ...
      if (psp->selection_multitracing == _FALSE_) {
	    ...
	    for (index_d1=0; index_d1<psp->d_size; index_d1++) {
	      for (index_d2=MAX(0,index_d1-psp->non_diag); index_d2<=MIN(index_d1+psp->non_diag,psp->d_size-1); index_d2++) {
	  	    cl_integrand[index_q*cl_integrand_num_columns+1+psp->index_ct_dd+index_ct]=
	  	     primordial_pk[index_ic1_ic2]
	  	     * transfer_ic1_nc[index_d1]
	  	     * transfer_ic2_nc[index_d2]
