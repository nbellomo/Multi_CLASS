  ...
  if ((ppt->has_cl_number_count == _TRUE_) || (ppt->has_cl_lensing_potential == _TRUE_)) {
	...
	if (flag1 == _TRUE_) {
        if (strstr(string1,"yes") != NULL) {
          ptr->selection_tracers_number = 2;
          ptr->selection_multitracing = _TRUE_;
          psp->selection_multitracing = ptr->selection_multitracing;
        }
        else if (strstr(string1,"no") != NULL) {
          ptr->selection_tracers_number = 1;
          ptr->selection_multitracing = _FALSE_;
          psp->selection_multitracing = ptr->selection_multitracing;
        }
        else {
          class_stop(errmsg, "In selection_multitracing input: option %s is unclear.", string1);
        }
    }
    
    /** The single and multiple tracers are now treated on "equal ground". 
     *  Changes introduced in the structure to eliminate ambiguity/redundancies. */
	...
	if ((flag1 == _TRUE_) && (int1>0)) {
	  ...
	  class_alloc(ptr->selection_mean, sizeof(double)*ptr->selection_num, errmsg);
	  for (i=0; i<ptr->selection_num; i++) {
		class_test((pointer1[i] < 0.) || (pointer1[i] > 1000.),
				   errmsg,
				   "You asked for a mean redshift equal to %e, sounds odd.", pointer1[i]);
		ptr->selection_mean[i] = pointer1[i];
		if (i>0) {
		  class_test(ptr->selection_mean[i] <= ptr->selection_mean[i-1],
					 errmsg,
					 "List of mean redshifts not be passed in strictly growing order.");
		}
	  }
	  /** This line might be re-thought when the different bins option will be included. */
	  ppt->smallest_redshift_bin = ptr->selection_mean[0];
	  free(pointer1);
	}
	else {
	  class_stop(errmsg, "No mean redshift provided.");
	}
	...
	if ((flag1 == _TRUE_) && (int1>0)) {
	  class_test(int1 != ptr->selection_num,
				 errmsg,
				 "Number of mean redshift and redshift bin width do not match.");
	  class_alloc(ptr->selection_width, sizeof(double)*ptr->selection_num, errmsg);
	  for (i=0; i<ptr->selection_num; i++) {
	  ...
	  class_stop(errmsg, "No redshift bin width provided.");
	}
	...
	if (flag1 == _TRUE_) {
	  if (strstr(string1,"gaussian") != NULL) {
		ptr->selection_window = gaussian;
	  }
	  else if (strstr(string1,"tophat") != NULL) {
		ptr->selection_window = tophat;
	  }
	  ...
	  class_stop(errmsg, "No window function provided.");
	}
	...
	if ((flag1 == _TRUE_) && (int1>0)) {
	  class_test(int1 != ptr->selection_num * ptr->selection_tracers_number,
				 errmsg,
				 "Incorrect number of bias values.");
	  class_alloc(ptr->selection_bias, sizeof(double)*ptr->selection_num*ptr->selection_tracers_number, errmsg);
	  for (i=0; i<ptr->selection_num*ptr->selection_tracers_number; i++) {
		ptr->selection_bias[i] = pointer1[i];
	  }
	  free(pointer1);
	  
	  /* Primordial non-Gaussianity effect on bias. */
	  class_read_double("f_NL", ptr->f_NL);
      class_read_double("n_NG", ptr->n_NG);
      class_read_double("k_pivot_NG", ptr->k_pivot_NG);
	}
	else {
	  if (ppt->has_nc_density == _TRUE_) {
		class_stop(errmsg, "No bias has been provided.");
	  }
	}
	...
	if ((flag1 == _TRUE_) && (int1>0)) {
	  class_test(int1 != ptr->selection_num * ptr->selection_tracers_number,
				 errmsg,
				 "Number of mean redshift and magnification bias values do not match.");
	  class_alloc(ptr->selection_magnification_bias, sizeof(double)*ptr->selection_num*ptr->selection_tracers_number, errmsg);
	  for (i=0; i<ptr->selection_num*ptr->selection_tracers_number; i++) {
	  ...
	  if ((ppt->has_nc_rsd == _TRUE_ || ppt->has_nc_lens == _TRUE_ || ppt->has_nc_gr == _TRUE_)) {
	...
	if (ptr->selection_multitracing == _TRUE_) {
	  ...
	  if (flag1 == _FALSE_ || flag2 == _FALSE_) {
	    class_stop(errmsg, "At least one between 'selection_dNdz_1' and 'selection_dNdz_2' has not been specified.");
	  }
	  		     
	  if (flag1 == _TRUE_ && flag2 == _TRUE_) {
		
		if ((strstr(string1,"analytic") == NULL && strstr(string1,"file") == NULL) ||
		    (strstr(string2,"analytic") == NULL && strstr(string2,"file") == NULL)) {
		  class_stop(errmsg, "At least one between 'selection_dNdz_1' and 'selection_dNdz_2' has not been recognized.");	
		}
		
	    if ((strstr(string1,"analytic") != NULL && strstr(string2,"file") != NULL) ||
	        (strstr(string1,"file") != NULL && strstr(string2,"analytic") != NULL)) {
	  	  class_stop(errmsg, "The current version of Multi_CLASS does not support mixed input for the dNdz. Choose only 'analytic' or 'file' for both tracers.");	  
	    }
	  }
	}
	...
	if (flag1 == _TRUE_) {
	  if (strstr(string1,"analytic") != NULL){
	    ...
	    if (flag1 == _TRUE_) {
    	  if (strstr(string1,"euclid_galaxy") != NULL) {
    		ptr->selection_tracer_1 = euclid_galaxy;
    	  }
    	  else if (strstr(string1,"spherex_galaxy") != NULL) {
    		ptr->selection_tracer_1 = spherex_galaxy;
    	  }
    	  else if (strstr(string1,"astrophysical_gws") != NULL) {
    		ptr->selection_tracer_1 = astrophysical_gws;
    	  }
    	  else {
    		class_stop(errmsg,"In selection_tracer_1 input: type '%s' is unclear.",string1);
    	  }
    	}
    	else {
    	  class_stop(errmsg, "The name of the first tracer has not been recognized.");
    	}
    	...
    	if (ptr->selection_multitracing == _TRUE_) {
	      ...
	      if (flag2 == _TRUE_) {
    	    if (strstr(string2,"euclid_galaxy") != NULL) {
    	  	  ptr->selection_tracer_2 = euclid_galaxy;
    	    }
    	    else if (strstr(string2,"spherex_galaxy") != NULL) {
    	  	  ptr->selection_tracer_2 = spherex_galaxy;
    	    }
    	    else if (strstr(string2,"astrophysical_gws") != NULL) {
    		  ptr->selection_tracer_2 = astrophysical_gws;
    	    }
    	    else {
    	  	class_stop(errmsg,"In selection_tracer_2 input: type '%s' is unclear.",string1);
    	    }
    	  }
    	  else {
    	    class_stop(errmsg, "The name of the second tracer has not been recognized.");
    	  }
		}  
	  }
	  else if (strstr(string1,"file") != NULL) {
		ptr->has_nz_file = _TRUE_;
		ptr->has_nz_evo_file = _TRUE_;
		
		class_read_string("selection_dNdz_filepath_1", ptr->nz_file_name_1);
		class_read_string("selection_dNdzevolution_filepath_1", ptr->nz_evo_file_name_1);

		if (ptr->selection_multitracing == _TRUE_) {
          class_read_string("selection_dNdz_filepath_2", ptr->nz_file_name_2);
		  class_read_string("selection_dNdzevolution_filepath_2", ptr->nz_evo_file_name_2);
		}
	  }
	  else {
		class_stop(errmsg, "No 'analytic' or 'file' input has been provided.");
	  }
	}        
  ...
  if ((ppt->has_pk_matter == _TRUE_) || (ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_cl_number_count == _TRUE_) || (ppt->has_cl_lensing_potential == _TRUE_)) {
    ...
    if (flag1==_TRUE_) {
      ...
      if ((ppt->has_cl_number_count == _TRUE_) || (ppt->has_cl_lensing_potential == _TRUE_)) {
        ...
        for (bin=0; bin<ptr->selection_num; bin++) {
          ...
          if (ptr->selection_window==gaussian) {
            z_max = ptr->selection_mean[bin]+ptr->selection_width[bin]*ppr->selection_cut_at_sigma;
          }
          if (ptr->selection_window==tophat) {
            z_max = ptr->selection_mean[bin]+(1.+ppr->selection_cut_at_sigma*ppr->selection_tophat_edge)*ptr->selection_width[bin];
          }
...
int input_default_params(
  ...
  psp->selection_multitracing = _FALSE_;
